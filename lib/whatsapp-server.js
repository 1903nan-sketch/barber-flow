...
export async function requireWhatsappSettingsAccess(req,tenantId){
  ...
  const {data:m}=await sb.from("memberships")
    .select("role,permissions,active,tenants(status,plans(name))")
  ...
  if(String(m?.tenants?.plans?.name||"").toLowerCase()==="starter")return {error:"O robô do WhatsApp está disponível nos planos Pro e Premium.",status:403};

...
export function useWorkspace(){
 ...
 useEffect(()=>{let alive=true; async function load(){
  ...
  if(alive)setState({loading:false,user,membership,tenant:membership?.tenants||null,error:error?.message||(!membership?"Sua conta ainda não foi vinculada a uma barbearia.":"")});

...
export function useWorkspace(){
 ...
 useEffect(()=>{let alive=true; async function load(){
  ...
  const list=(memberships||[]).filter(m=>m.tenants?.product_slug==="beautytix"),preferred=localStorage.getItem("beautytix_workspace"),membership=list.find(m=>m.tenant_id===preferred)||list[0]||null;
  if(membership)localStorage.setItem("beautytix_workspace",membership.tenant_id);
  const tenant=membership?.tenants||null,blocked=tenant&&!["trial","active","pending","overdue"].includes(tenant.status);
  if(alive)setState({loading:false,user,membership,memberships:list,tenant,error:error?.message||(!membership?"Sua conta ainda não foi vinculada a um negócio BeautyTix.":blocked?"O acesso deste negócio está temporariamente indisponível. Fale com o suporte BeautyTix.":"")});

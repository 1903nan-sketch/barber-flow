...
export function useWorkspace(){
 ...
 useEffect(()=>{let alive=true; async function load(){
  ...
  const {data:memberships,error}=await supabase.from("memberships").select("tenant_id,name,role,permissions,active,tenants(*,plans(name,monthly_cents,max_barbers,max_units))").eq("user_id",user.id).eq("active",true).order("name");

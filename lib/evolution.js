...
export function evolutionInstanceName(tenantId){
  return "beautytix-"+String(tenantId||"").toLowerCase().replace(/[^a-z0-9]/g,"");
}

export function tenantIdFromEvolutionInstance(instanceName){
  const match=String(instanceName||"").toLowerCase().match(/^beautytix-([0-9a-f]{32})$/);
  if(!match)return "";
  const h=match[1];

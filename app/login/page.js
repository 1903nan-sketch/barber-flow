...
export default function LoginPage(){
 ...
 return <main className={styles.page}>
  <section className={styles.brand}><div className={styles.logo}><Scissors aria-hidden="true"/>BarberTix</div><div><span className={styles.kicker}>GESTÃO QUE TRANSFORMA</span><h1>Mais controle.<br/>Mais clientes.<br/><em>Mais crescimento.</em></h1><p>Administre sua barbearia de qualquer lugar, com tudo o que você precisa em um só sistema.</p></div><small>BarberTix · by Ruptix</small></section>
  <section className={styles.panel} aria-label="Acesso ao BarberTix"><form className={styles.card} onSubmit={submit} aria-busy={loading}>
   <div className={styles.mobileLogo+' '+styles.logo}><Scissors aria-hidden="true"/>BarberTix</div><span className={styles.welcome}>{mode==='login'?'BEM-VINDO DE VOLTA':'SUA CONTA'}</span><h2>{titles[mode]}</h2><p className={styles.intro}>{mode==='reset'?'Informe seu e-mail para receber o link de recuperação.':mode==='new-password'?'Escolha uma nova senha com pelo menos 6 caracteres.':mode==='signup'?'Use seu e-mail para criar o acesso.':'Seu negócio organizado começa aqui.'}</p>

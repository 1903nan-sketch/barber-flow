...
export default function LoginPage(){
 ...
 async function submit(event){
  ...
  try{
   ...
   if(mode==='login'&&email.startsWith('@')){const {data:matches,error:lookupError}=await supabase.from('staff_logins').select('login_email').eq('username',email.slice(1).toLowerCase()).limit(2);if(lookupError||matches?.length!==1)throw new Error('E-mail, usuário ou senha incorretos.');email=matches[0].login_email}
 ...
 return <main className={styles.page}>
  <section className={styles.brand}><div className={styles.logo}><Scissors aria-hidden="true"/>Barber Flow</div><div><span className={styles.kicker}>GESTÃO QUE TRANSFORMA</span><h1>Mais controle.<br/>Mais clientes.<br/><em>Mais crescimento.</em></h1><p>Administre sua barbearia de qualquer lugar, com tudo o que você precisa em um só sistema.</p></div><small>Barber Flow · by Ruptix</small></section>
  <section className={styles.panel} aria-label="Acesso ao Barber Flow"><form className={styles.card} onSubmit={submit} aria-busy={loading}>
   <div className={styles.mobileLogo+' '+styles.logo}><Scissors aria-hidden="true"/>Barber Flow</div><span className={styles.welcome}>{mode==='login'?'BEM-VINDO DE VOLTA':'SUA CONTA'}</span><h2>{titles[mode]}</h2><p className={styles.intro}>{mode==='reset'?'Informe seu e-mail para receber o link de recuperação.':mode==='new-password'?'Escolha uma nova senha com pelo menos 6 caracteres.':mode==='signup'?'Use seu e-mail para criar o acesso.':'Seu negócio organizado começa aqui.'}</p>
   ...
   <p className={styles.footer}>BARBER FLOW · BY RUPTIX</p>
  </form></section>
 </main>
}
